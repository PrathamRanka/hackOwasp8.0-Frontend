...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const isMobile = useMemo(() => {
    ...
    const hasTouchScreen = 'ontouchstart' in window || window.navigator.maxTouchPoints > 0;
    const isSmallScreen = window.innerWidth <= 768;
    const windowWithOpera = window as Window & { opera?: string };
    const userAgent = window.navigator.userAgent || window.navigator.vendor || windowWithOpera.opera || '';
  ...
  useEffect(() => {
    ...
    const activeAreaElement = activeAreaSelector
      ? (document.querySelector(activeAreaSelector) as HTMLElement | null)
      : null;
    const originalBodyCursor = document.body.style.cursor;
    const originalAreaCursor = activeAreaElement?.style.cursor;

    if (hideDefaultCursor) {
      if (activeAreaElement) {
        activeAreaElement.style.cursor = 'none';
      } else {
    ...
    gsap.set(cursor, { autoAlpha: activeAreaElement ? 0 : 1 });
    ...
    const moveHandler = (e: MouseEvent) => {
      if (activeAreaSelector) {
        const target = e.target as Element | null;
        const isInsideArea = !!target?.closest(activeAreaSelector);
    ...
    const enterHandler = (e: MouseEvent) => {
      if (activeAreaSelector) {
        const scopedTarget = e.target as Element | null;
        if (!scopedTarget?.closest(activeAreaSelector)) {
          return;
        }
      }
    ...
    return () => {
      ...
      if (hideDefaultCursor) {
        if (activeAreaElement) {
          activeAreaElement.style.cursor = originalAreaCursor ?? '';
        } else {
  ...
  }, [targetSelector, activeAreaSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isMobile, hoverDuration, parallaxOn]);
